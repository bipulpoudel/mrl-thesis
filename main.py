...
def train_model(model, device, train_loader, criterion, reservoir_buffer, epochs=Hyperparameters.EPOCHS_PER_TASK):

    optimizer = optim.SGD(model.parameters(), lr=Hyperparameters.LEARNING_RATE, 
                         momentum=0.9, weight_decay=5e-4)
    scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=30, gamma=0.1)  # Reduce LR every 30 epochs
...
if __name__ == '__main__':
    ...
    for i, (train_loader, test_loader) in enumerate(zip(train_loaders, test_loaders)):
        ...
        #Train the model
        train_model(model, device, train_loader, criterion, reservoir_buffer, epochs=Hyperparameters.EPOCHS_PER_TASK)
